...
} from "@/app/utils/constant/navigations/publicNavigationLinks";
...
const MobileNav = () => {
  ...
  const linksToShow: INavigationLinks[] = publicNavigationLinks.filter(
    (link) => {
      if (token) {
        return true;
      } else {
  ...
  const { platformLogo, isLoading } = useAppConfig();
